...
// @sect3{The WaveSolver class Template}
// We then define the main class that solves the Wave problem.
...
namespace Step101
{
  ...
  // ==================================================================
  template <int dim>
  class DerivativeInitialCondition : public Function<dim>
  {
  public:
    double value(const Point<dim>  &p,
                        const unsigned int component = 0) const override;
  ...
  template <int dim>
  double DerivativeInitialCondition<dim>::value(const Point<dim> &p,
                                      const unsigned int component) const
  {
    AssertIndexRange(component, this->n_components);
    (void)component;
  ...
  template <int dim>
  bool WaveSolver<dim>::face_has_ghost_penalty(
    const typename Triangulation<dim>::active_cell_iterator &cell,
    const unsigned int                                       face_index) const
  {
                    ...
                    for (unsigned int j = 0; j < n_interface_dofs; ++j)
                      {
                        ...
                        local_mass_stabilization(i, j) +=
                          .5 * ghost_parameter_1 * (0.20) * std::pow(cell_side_length,5) * normal *
                        ...
                        local_stabilization(i, j) +=
                          ...
                          fe_interface_values.JxW(q);
                        local_stabilization(i, j) +=
                          .5 * ghost_parameter_2 * (0.20) *  std::pow(cell_side_length,3) * normal *
                          fe_interface_values.jump_in_shape_hessians(i, q) * normal *
                          normal *
                          fe_interface_values.jump_in_shape_hessians(j, q) * normal *
                          fe_interface_values.JxW(q);
  ...
  void WaveSolver<dim>::output_results() const
  {
    ...
    std::ofstream output("step-101.vtu");
    ...
    for (unsigned int cycle = 0; cycle <= n_refinements; cycle++)
      {
        ...
        while(time<final_time-1e-6)
        {
          ...
          sol_k1 = old_derivative_solution;

          // k2
          sol_k2 = sol_k1;
          sol_k2.add(time_step / 2.0, der_sol_k1);
          der_tmp = old_solution;
          der_tmp.add(time_step / 2.0, sol_k1);
          solve(time + time_step / 2.0, der_tmp, der_sol_k2);

          // k3
          sol_k3 = sol_k1;
          sol_k3.add(time_step / 2.0, der_sol_k2);
          der_tmp = old_solution;
          der_tmp.add(time_step / 2.0, sol_k2);
          solve(time + time_step / 2.0, der_tmp, der_sol_k3);

          // k4
          sol_k4 = sol_k1;
          sol_k4.add(time_step, der_sol_k3);
          der_tmp = old_solution;
          der_tmp.add(time_step, sol_k3);
          solve(time + time_step, der_tmp, der_sol_k4);
          ...
          derivative_solution = old_derivative_solution;
          derivative_solution.add(time_step / 6.0, der_sol_k1);
          derivative_solution.add(time_step / 3.0, der_sol_k2);   // 2/6 = 1/3
          derivative_solution.add(time_step / 3.0, der_sol_k3);
          derivative_solution.add(time_step / 6.0, der_sol_k4);
